...
import { Flame, Clock, Soup, CookingPot } from 'lucide-react';
      ...
      {currentRecipeList.map((recipe, index) => (
        ...
        >
          ...
          <div className="ml-4"> {/* Adds space to accommodate the left-side icon */}
            ...
            <p className="text-yellow-600 mb-2">{recipe.rating}</p>

            {/* Icons for kcal, time, protein */}
            <div className="flex items-center space-x-4 mb-4">
              <div className="flex flex-col items-center text-xs text-slate-700">
                <Flame className="w-5 h-5 text-[#00a39e] mb-1" />
                <span> {recipe.calories}</span>
              </div>
              <div className="flex flex-col items-center text-xs text-slate-700">
                <Clock className="w-5 h-5 text-[#00a39e] mb-1" />
                <span> {recipe.cookTime} </span>
              </div>
              <div className="flex flex-col items-center text-xs text-slate-700">
                <Soup className="w-5 h-5 text-[#00a39e] mb-1" />
            ...
            >
              <button className="mt-4 p-2 px-6 bg-[#00a39e] border border-[#00f5d0] shadow-lg ring-1 ring-black/5 rounded-full text-white flex items-center gap-2">
                View Recipe
